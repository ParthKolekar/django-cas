...
def _verify_simple(ticket, service):
    """
...
def _verify_with_attributes(ticket, service):
    """
        Verifies CAS XML-based authentication ticket and returns extended attributes.
        Returns username on success and None on failure.
    """
...
_PROTOCOLS = {'1': _verify_simple, '2': _verify_with_attributes, '3': _verify_with_attributes, 'CAS_2_SAML_1_0': _verify_cas2_saml}
...
class CASBackend(object):
    ...
    def authenticate(self, ticket, service, request):
        ...
        logger.debug('Attempting to validate ' + str(ticket))
        username, attributes = _verify(ticket, service)
        logger.debug('Authenticated ' + str(username))
        if attributes:
